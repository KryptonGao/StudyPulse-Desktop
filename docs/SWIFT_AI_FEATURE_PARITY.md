...
> 桌面端实现状态（2026-08-11）：已完成。错题详情现在复用 `MistakeAnalysis` caller，通过 `kind` 分派结构化解析、相似题、自测题批改、思维导图、辩论、知识断层、图片识别和 AI OCR。结果默认停留在预览态；错题字段通过 Core 局部 patch 写回，题组/批改/导图/辩论等结果写入错题的受限扩展字段，修复任务需用户确认后才创建。图片只允许写入 `Media/images` 并通过多模态 provider 发送，桌面端 OCR 采用 provider OCR，不生成虚假的本地 OCR 结果。
