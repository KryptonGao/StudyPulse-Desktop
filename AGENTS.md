...
采用基于风险的验证方式。不要在每次变更后都自动运行完整的回归测试套件。

## 1. 首先对变更进行分类

在进行验证之前，将变更归类为以下类别之一：
...
- 检查代码差异（diff）
- 仅在直接相关时运行格式化/代码检查（lint）
- 仅在修改的代码可能影响编译时运行构建/类型检查
- **不**运行回归测试
...
- 运行最小范围的相关单元测试/组件测试
- 在有必要时，针对受影响的包（package）或目标（target）运行类型检查/构建
- 除非针对性验证显示存在不确定性或失败，否则**不**运行完整的回归测试套件
...
- 首先运行针对性测试
- 随后运行覆盖受影响系统的更广泛的回归测试
- 当影响范围（blast radius）波及整个仓库或难以界定时，运行完整的回归测试套件

## 2. 升级验证级别的规则

从能够合理检测出错误的、成本最低的验证方式开始。
...
- 多个子系统受到影响
- 行为跨越了模块/包的边界
- 变更是为了修复回归问题（regression）
- 变更修改了不变性约束（invariant）或公共契约（public contract）
- 先前的检查揭示了意外的副作用

不要仅仅因为有更多可用的测试就升级验证级别。 ## 3. 完整回归测试套件

在满足以下至少一项条件时，应运行完整的回归测试套件：
...
微小的视觉变更只需进行微小的验证。
小幅的行为变更只需进行针对性验证。
涉及多处或高风险的变更则需进行广泛的回归验证。

不要仅仅为了“求稳”而运行不相关的测试。
除非明确要求，否则不要修复不相关的测试失败。
