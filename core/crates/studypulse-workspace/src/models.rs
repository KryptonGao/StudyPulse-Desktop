...
/// A durable checkpoint for a long-running Agent turn.
///
/// The Workspace crate deliberately stores the mode and status as strings so
/// it does not depend on the Agent runtime crate. The Agent owns the meaning of
/// those values and validates them before starting or resuming work.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
impl AgentTurn {
    pub fn validate(&self) -> crate::Result<()> {
        ...
        {
            return Err(crate::WorkspaceError::MalformedData {
                path: "Agent/turns".into(),
                detail: "turn id is invalid".into(),
            });
        }
        if self.mode.trim().is_empty() || self.goal.trim().is_empty() {
            return Err(crate::WorkspaceError::MalformedData {
                path: format!("Agent/turns/{}.json", self.id),
                detail: "turn mode and goal must not be empty".into(),
            });
        }
        for timestamp in [&self.created_at, &self.updated_at] {
            chrono::DateTime::parse_from_rfc3339(timestamp).map_err(|error| {
                crate::WorkspaceError::MalformedData {
                    path: format!("Agent/turns/{}.json", self.id),
                    detail: format!("invalid turn timestamp: {error}"),
                }
            })?;
        }
        if self.history_json.len() > 512 * 1024
            || self
                .result_json
                .as_ref()
                ...
                .is_some_and(|value| value.len() > 64 * 1024)
        {
            return Err(crate::WorkspaceError::MalformedData {
                path: format!("Agent/turns/{}.json", self.id),
                detail: "turn payload exceeds 512 KiB".into(),
            });
        }
        Ok(())
    }
