...
export const core = {
  ...
  applyMistakeAiPatch: (id: string, patch: { error_reason?: string; wrong_solution?: string; correct_solution?: string; tags?: string[]; question_images?: string[]; result?: unknown }) =>
    command<MistakeNote>("apply_mistake_ai_patch", { id, patch_json: JSON.stringify(patch) }),
  saveMistakeAiSession: (id: string, kind: string, payload: unknown) =>
    command<void>("save_mistake_ai_session", { id, kind, payload_json: JSON.stringify(payload) }),
  ...
  readMedia: (relativePath: string) => command<string>("read_media", { relative_path: relativePath }),
  writeMedia: (relativePath: string, dataBase64: string) => command<string>("write_media", { relative_path: relativePath, data_base64: dataBase64 }),
